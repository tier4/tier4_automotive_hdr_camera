...
class Type(Enum):
    ...
    @property
    def is_tier4mp(self) -> bool:
        return self in (Type.C1MP, Type.C2MP, Type.C3MP)
...
def _camera_gmsl_serializer(index: int, suffix: StringLike, vc_id: StringLike,
    ...
    if primary:
        ser_prim = gmsl.Serializer.primary(index)
        ser = gmsl.Serializer.secondary_a(index, suffix, vc_id, dser_label)
        ser_prim.properties(['xerror-gpios = <7>'])
        ser.properties(['xerror-gpios = <7>'])
        return (ser_prim, ser)
    else:
        ser = gmsl.Serializer.secondary_b(index, suffix, vc_id, dser_label)
        ser.properties(['xerror-gpios = <7>'])
        return (None, ser)


def _tier4mp_gmsl_serializer(index: int, suffix: StringLike, vc_id: StringLike,
                              dser_label: str, primary: bool
                              ) -> Tuple[Optional[DeviceTreeNode], DeviceTreeNode]:
    """Tier4MP serializer helper for Anvil platform"""
    if primary:
        ser_prim = gmsl.Tier4MPSerializer.primary(index)
        ser = gmsl.Tier4MPSerializer.secondary_a(index, suffix, vc_id, dser_label)
        ser_prim.properties(['xerror-gpios = <5>'])
        ser.properties(['xerror-gpios = <5>'])
        return (ser_prim, ser)
    else:
        ser = gmsl.Tier4MPSerializer.secondary_b(index, suffix, vc_id, dser_label)
        ser.properties(['xerror-gpios = <5>'])
        return (None, ser)


def _tier4mp_isp(index: int, suffix: StringLike, vc_id: StringLike,
                  dser_label: str, primary: bool
                  ) -> Tuple[Optional[DeviceTreeNode], DeviceTreeNode]:
    """Tier4MP ISP (GW5300) helper for Anvil platform - special i2c@0 handling"""
    isp_prim = isp = None
...
def _tier4mp_camera_gmsl_serializer(index: int, suffix: StringLike, vc_id: StringLike,
                            dser_label: str, primary: bool
                            ) -> Tuple[Optional[DeviceTreeNode], DeviceTreeNode]:
    """Tier4MP serializer helper for DevKit platform"""
    if primary:
        ser_prim = gmsl.Tier4MPSerializer.primary(index)
        ser = gmsl.Tier4MPSerializer.secondary_a(index, suffix, vc_id, dser_label)
        ser_prim.properties(['xerror-gpios = <5>'])
...
def _tier4mp_camera_isp(index: int, suffix: StringLike, vc_id: StringLike,
                dser_label: str, primary: bool
                ) -> Tuple[Optional[DeviceTreeNode], DeviceTreeNode]:
    """Tier4MP ISP (GW5300) helper for Devkit platform - uses @6d for all isp_b"""
    isp_prim = isp = None

    if primary:
        isp_prim = (DeviceTreeNode('gw5300_prim@6c', label=f'isp{index}_prim')
            .properties([
                'status = "okay"',
                'compatible = "nvidia,tier4mp_gw5300"',
                'reg = <0x6c>',
        ...
        isp = (DeviceTreeNode(f'gw5300_{suffix}@6d', label=f'isp{index}_a')
            .properties([
                'compatible = "nvidia,tier4mp_gw5300"',
                'status = "okay"',
                'reg = <0x6d>',
        ...
        isp_prim = None
        isp = (DeviceTreeNode(f'gw5300_{suffix}@6d', label=f'isp{index}_b')
            .properties([
                'compatible = "nvidia,tier4mp_gw5300"',
                'status = "okay"',
                'reg = <0x6d>',
...
class C1MP(Camera):
    ...
    def __init__(self, index: int, suffix: StringLike, address: str, vc_id: StringLike,
        ...
        self.ser_prim, self.ser = _tier4mp_gmsl_serializer(index, suffix, vc_id, dser_label, primary)
            ...
            .nodes([
                DeviceTreeNode('mode0')
                    ...
                    .properties([
                        ...
                        'line_length = "2200"',
                        'inherent_gain = "1"',
                        f'pix_clk_hz = "{serdes_pix_clk_hz}"',
                        'gain_factor = "10"',
                        'min_gain_val = "0"',
                        'max_gain_val = "300"',
                        'step_gain_val = "3"',
...
class C2MP(Camera):
    ...
    def __init__(self, index: int, suffix: StringLike, address: str, vc_id: StringLike,
        ...
        self.ser_prim, self.ser = _tier4mp_gmsl_serializer(index, suffix, vc_id, dser_label, primary)
        self.isp_prim, self.isp = _tier4mp_isp(index, suffix, vc_id, dser_label, primary)
...
class C3MP(Camera):
    """C3MP camera with IMX728 sensor using tier4mp drivers"""
    ...
    def __init__(self, index: int, suffix: StringLike, address: str, vc_id: StringLike,
        ...
        self._image_sensor = (DeviceTreeNode(f'imx728_{suffix}@{address}')
            ...
            .nodes([
                DeviceTreeNode('mode0')
                    .header('\n/* mode IMX728_MODE_3840X2160_CROP_30FPS */')
                    .properties([
                        'mclk_khz = "24000"',
                        f'num_lanes = "{csi_lanes}"',
                        f'tegra_sinterface = "{tegra_sinterface}"',

...
//  struct  tier4_imx490    *next_client_priv;
    ...
    for( i = 0 ; i < camera_channel_count ; i++ ) {
        ...
        if ( i & 0x1 ) { // if  i = 1,3,5,7 ( GMSL B port of a Des )
            ...
            if ( tier4_imx490_is_camera_connected_to_port(i) ) { //  a  camera is connected to GMSL B portL

                if (    ( tier4_imx490_is_current_port(priv, i) == true )
                    &&  ( tier4_imx490_is_camera_running_on_port(i) == false )) {
...
//                  wst_priv[i].running  = true;
            ...
            if (    ( tier4_imx490_is_camera_connected_to_port(i) == true )
                ...
                if ( tier4_imx490_check_null_tc_dev_for_port(i+1) ) { // check if tc_dev is null
...
error_exit:
    ...
    dev_info(dev,"[%s] :camera_channel_count = %d  p_client = %p \n", __func__, camera_channel_count, wst_priv[camera_channel_count].p_client );
    ...
    for( i = 0 ; i < camera_channel_count ; i++ ) {
        ...
        if ( tier4_imx490_is_current_i2c_client(client, i)) {
            ...
            if ( i & 0x1 ) { // Even port number( GMSL B port on a Des : i = port_number -1 )
                ...
                if (  tier4_imx490_is_camera_connected_to_port(i-1)) {              // if a camera connected to another(GMSL A)port on a Des.
                ...
                if ( tier4_imx490_is_camera_connected_to_port(i+1)) {               // Another camera is connected to another(GMSL B) port on the Des
